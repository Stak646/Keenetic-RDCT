#!/bin/sh
# Collector: services.entware — Entware Services
set -eu
...
if [ -d /opt/etc/init.d ]; then
  ls -la /opt/etc/init.d/ > "$ARTIFACTS/initd_list.txt" 2>/dev/null && cmds_run=$((cmds_run+1))
  for svc in /opt/etc/init.d/S*; do
    [ -f "$svc" ] || continue
    sname=$(basename "$svc")
    head -c 2048 "$svc" > "$ARTIFACTS/svc_${sname}.txt" 2>/dev/null
    # Try to get status
    "$svc" status > "$ARTIFACTS/svc_${sname}_status.txt" 2>/dev/null || true
    cmds_run=$((cmds_run+1))
  done
else
  status="SKIP"
...
cat > "$RESULT" << RESEOF
{
  ...
  "collector_id":"services.entware","status":"$status",
