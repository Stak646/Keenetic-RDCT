...
### DATA ###
collect "iptables-save" "iptables_save.txt" "iptables -L -n -v"
collect "ip6tables-save" "ip6tables_save.txt" "ip6tables -L -n -v"
command -v nft >/dev/null 2>&1 && collect "nft list ruleset" "nft_ruleset.txt"
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"network.firewall","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
