...
### DATA ###
collect "df -hT" "df.txt" "df -h"
collect "mount" "mounts.txt" "cat /proc/mounts"
collect "lsblk -f" "lsblk.txt" "cat /proc/partitions"
read_file "/proc/partitions" "partitions.txt"
read_file "/proc/diskstats" "diskstats.txt"
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"storage.map","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
