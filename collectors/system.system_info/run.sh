#!/bin/sh
# Collector: system.system_info — System Info
set -eu
...
### DATA COLLECTION ###
collect "uname -a" "uname.txt"
collect "cat /proc/version" "version.txt"
read_file "/proc/cpuinfo" "cpuinfo.txt"
read_file "/proc/meminfo" "meminfo.txt"
read_file "/proc/uptime" "uptime.txt"
collect "df -h" "df.txt" "df"
collect "mount" "mounts.txt" "cat /proc/mounts"
[ -f /sys/class/thermal/thermal_zone0/temp ] && read_file "/sys/class/thermal/thermal_zone0/temp" "temperature.txt"
...
cat > "$RESULT" << RESEOF
{
  ...
  "collector_id":"system.system_info","status":"$status",
