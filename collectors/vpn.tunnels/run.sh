...
### DATA ###
collect "wg show all" "wg_show.txt"
collect "ip tunnel show" "ip_tunnel.txt"
collect "ip xfrm state" "ipsec_state.txt"
collect "ip xfrm policy" "ipsec_policy.txt"
for logf in /var/log/openvpn*.log /opt/var/log/openvpn*.log; do
  [ -r "$logf" ] && tail -c 65536 "$logf" > "$ARTIFACTS/$(basename $logf)" 2>/dev/null && cmds_run=$((cmds_run+1))
done
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"vpn.tunnels","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
