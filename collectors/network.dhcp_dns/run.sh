...
### DATA ###
read_file "/etc/resolv.conf" "resolv.conf"
for lf in /tmp/dhcp.leases /var/lib/misc/dnsmasq.leases /opt/var/lib/misc/dnsmasq.leases; do
  [ -r "$lf" ] && cp "$lf" "$ARTIFACTS/$(basename $lf)" 2>/dev/null && cmds_run=$((cmds_run+1))
done
read_file "/opt/etc/dnsmasq.conf" "dnsmasq.conf"
collect "cat /etc/hosts" "hosts.txt"
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"network.dhcp_dns","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
