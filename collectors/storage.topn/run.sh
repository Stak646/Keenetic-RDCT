...
### DATA ###
collect "df -i" "inode_usage.txt"
du -sh /opt/* 2>/dev/null | sort -rh | head -30 > "$ARTIFACTS/top_dirs_opt.txt" && cmds_run=$((cmds_run+1))
find /opt -maxdepth 4 -type f -size +1M 2>/dev/null | head -100 | while read -r f; do
  du -sh "$f" 2>/dev/null
done | sort -rh > "$ARTIFACTS/large_files.txt" && cmds_run=$((cmds_run+1))
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"storage.topn","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
