...
### DATA ###
collect "ss -tulnp" "ss_tulnp.txt" "netstat -tulnp"
collect "ss -s" "ss_stats.txt" "netstat -s"
collect "ss -anp" "ss_all.txt" "netstat -anp"
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"network.sockets","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
