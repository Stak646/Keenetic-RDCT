...
### DATA COLLECTION ###
HOOK_DIRS="/opt/etc/ndm/netfilter.d /opt/etc/ndm/ifstatechanged.d /opt/etc/ndm/fs.d /opt/etc/ndm/schedule.d /opt/etc/ndm/neighbour.d"
for hd in $HOOK_DIRS; do
  if [ -d "$hd" ]; then
    dname=$(basename "$hd")
    ls -la "$hd" > "$ARTIFACTS/hooks_${dname}_list.txt" 2>/dev/null && cmds_run=$((cmds_run+1))
    for script in "$hd"/*; do
      [ -f "$script" ] && head -c 4096 "$script" > "$ARTIFACTS/hook_${dname}_$(basename "$script").txt" 2>/dev/null
    done
  fi
done
...
cat > "$RESULT" << RESEOF
{
  ...
  "collector_id":"hooks.ndm","status":"$status",
