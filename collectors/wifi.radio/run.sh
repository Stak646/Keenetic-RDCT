...
### DATA ###
collect "iwconfig" "iwconfig.txt" "iw dev"
collect "iw dev" "iw_dev.txt"
collect "iw phy" "iw_phy.txt"
collect "iw reg get" "iw_reg.txt"
...
cat > "$RESULT" << RESEOF
{"schema_id":"result","schema_version":"1","collector_id":"wifi.radio","status":"$status","started_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","finished_at":"$(date -u +%Y-%m-%dT%H:%M:%SZ 2>/dev/null)","duration_ms":0,"metrics":{"output_size_bytes":$out_bytes,"commands_run":$cmds_run,"commands_skipped":0,"commands_failed":$cmds_fail},"data":{},"artifacts":[${arts%,}],"errors":[],"fingerprint":"$fp"}
RESEOF
exit 0
